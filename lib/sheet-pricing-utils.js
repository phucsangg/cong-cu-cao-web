...
function cleanModelSpecs(model) {
    ...
    do {
        previous = cleaned;
        // Remove trailing parentheses containing arbitrary details like color
        cleaned = cleaned.replace(/\s*\([^)]*\)\s*$/, '').trim();
        // Remove dimensions like 70cm, 90cm, etc.
        cleaned = cleaned.replace(/[-_\s]*\d+\s*cm\b/i, '').trim();
        // Remove trailing color words and spec terms
        cleaned = cleaned.replace(/[-_\s]+(bạc|đen|trắng|xám|đỏ|gold|silver|black|white|grey|gray|new|inox|kính)\b/i, '').trim();

...
function collectPriceCandidates($) {
    ...
    const looksLikePriceText = (text) => {
        ...
        return /\b\d{1,3}(?:[.,]\d{3}){1,}\b/.test(value);
    };
...
function pickPriceFromScrapedProducts(products, model) {
    ...
    const enriched = (products || [])
        ...
        .filter((entry) => entry.parsedPrice);
...
async function processPricingRow({ row, deps = {} }) {
    ...
    if (!row || !row.brand || !String(row.brand).trim() || !row.model || !String(row.model).trim()) {
        return {
    ...
    return callAppsScript({
        ...
        payload: {
            ...
            updates: updates.map((update) => ({
                ...
                marketPrices: update.marketPrices || [],
...
function startBackgroundPricingJob({
    ...
    const job = {
        ...
        sheetName,
        appsScriptUrl,
        startRow: parseInt(startRow, 10) || 3,
        endRow: endRow ? parseInt(endRow, 10) : null,
        rowsConcurrency: Math.max(1, parseInt(rowsConcurrency, 10) || 2),
        linksConcurrency: Math.max(1, parseInt(linksConcurrency, 10) || 4),
    ...
    (async () => {
        ...
        try {
            ...
            job.rows = rows.map((row) => ({
                ...
                status: (row.brand && String(row.brand).trim()) && (row.model && String(row.model).trim()) ? 'pending' : 'skipped',
                marketPrices: [],
                minPrice: null,
                gapValue: null,
                gapPercent: null,
            ...
            const runnableRows = rows.filter((row) => (row.brand && String(row.brand).trim()) && (row.model && String(row.model).trim()));
            ...
            if (skippedRowsCount > 0) {
                log(`Bỏ qua ${skippedRowsCount} dòng do thiếu Thương hiệu hoặc Model.`);
            }
                ...
                while (!job.stopRequested) {
                    ...
                    try {
                        ...
                        if (result.status === 'success') {
                            job.successCount += 1;
                            log(`Dòng ${row.rowNumber} (${row.brand} ${row.model}) thành công: Min=${result.minPrice.toLocaleString('vi-VN')} đ, Đề xuất=${result.suggestedPrice ? result.suggestedPrice.toLocaleString('vi-VN') + ' đ' : '-'}`, 'success');
                        } else {
                            job.errorCount += 1;
                            log(`Dòng ${row.rowNumber} (${row.brand} ${row.model}) thiếu giá hợp lệ hoặc lỗi: ${result.status}`, 'warning');
                        }

...
function extractModelRegex(title = '') {
    const patterns = [
        /\b[A-Z]{2,}[0-9][A-Z0-9\-_]{2,}\b/g,
...
function cleanProductName(title = '', brand = '') {
    ...
    if (brand) {
        ...
        const brandRegex = new RegExp(`\\b${escapedBrand}\\b`, 'gi');
        text = text.replace(brandRegex, "");
    }

    text = text.replace(/\b(CHÍNH HÃNG|CHINH HANG)\b/gi, "");
    text = text.replace(/\bSERIE\b/gi, "");
...
function getModel(productName, brand, sku, barcode) {
    ...
    const regexModel = extractModelRegex(productName);
    if (regexModel) {
        return regexModel;
    }

    return cleanProductName(productName, brand);
}
...
async function syncHaravanIds({ appsScriptUrl, sheetUrl, haravanShopUrl, haravanAccessToken, fetchImpl = fetch }) {
    ...
    const shopUrl = String(haravanShopUrl).trim().replace(/\/$/, '');
    ...
    while (true) {
        ...
        for (const product of products) {
            ...
            for (const variant of variants) {
                ...
                const model = getModel(productName, brand, sku, barcode);

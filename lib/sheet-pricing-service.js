...
class CacheStore {
    constructor() {
        ...
        this.data = {
            keywords: {}, // { engine_q: { value, timestamp } }
            html: {},     // { url: { value, timestamp } }
            prices: {},   // { url_model: { value, timestamp } }
        };
        this.load();
    }
    ...
    load() {
        try {
            if (fs.existsSync(this.filePath)) {
                ...
                if (parsed.keywords && parsed.html && parsed.prices) {
                    this.data = parsed;
                }
            }
        } catch (e) {}
...
async function searchProductLinks({ brand, model, limit = 20, fetchImpl = fetch, sheetName, sourceMap }) {
    ...
    try {
        const queries = generateKeywords(brand, model, sheetName);
        const links = [];
        const seen = new Set();

        const addLinks = (newLinks, engine) => {
            newLinks.forEach((url) => {
                if (url && !seen.has(url)) {
                    seen.add(url);
                    links.push(url);
                    if (sourceMap) {
                        sourceMap.set(url, engine);
                    }
                }
            });
        };
        ...
        // Loop keywords in order
        for (let i = 0; i < Math.min(3, queries.length); i++) {
            const q = queries[i];
            if (i > 0 && hasEnoughLikelyLinks()) {
                break;
            }

            const fetchGoogleQuery = async (queryVal) => {
                const urls = [];
            ...
            const fetchBingQuery = async (queryVal) => {
                const urls = [];
            ...
            const fetchDDGQuery = async (queryVal) => {
                const urls = [];
                try {
            ...
            const fetchCocCocQuery = async (queryVal) => {
                const urls = [];
                try {
            ...
            const googleLinks = await getCachedSearchLinks('google', q, fetchGoogleQuery);
            addLinks(googleLinks, 'Google');

            const bingLinks = await getCachedSearchLinks('bing', q, fetchBingQuery);
            addLinks(bingLinks, 'Bing');

            const ddgLinks = await getCachedSearchLinks('ddg', q, fetchDDGQuery);
            addLinks(ddgLinks, 'DuckDuckGo');

            const coccocLinks = await getCachedSearchLinks('coccoc', q, fetchCocCocQuery);
            addLinks(coccocLinks, 'CocCoc');
...
function extractPriceAdvanced($, html, model, url, referencePrice) {
    ...
    // Handled in ld+json and microdata parsing.
...
async function extractProductPrice({ url, model, brand, referencePrice, fetchImpl = fetch }) {
    ...
    const cachedPrice = pricingCache.get('prices', cacheKey, 12);
    if (cachedPrice !== null) {
        return cachedPrice;
    }
    ...
    let html = pricingCache.get('html', url, 12);
    if (!html) {
        try {
            html = await fetchHtml(url, { fetchImpl, timeout: 8000 });
            pricingCache.set('html', url, html);
        } catch (error) {
            return null;
        }
    }
    ...
    pricingCache.set('prices', cacheKey, price);
    return price;
}
...
async function processPricingRow({ row, deps = {} }) {
    ...
    // Pass sourceMap to track search engine sources
    ...
    const allSitemapLinks = sitemapLinksResults.flat();

    // Merge search links and sitemap links
    const mergedLinks = Array.from(new Set([...discoveredLinks, ...allSitemapLinks]));

    // Filter by product detail url validation
    const allFilteredLinks = mergedLinks.filter((link) => isLikelyProductDetailUrl(link, row.model, row.brand));

    // Relevance scoring (Phần 3)
    const scoredLinks = allFilteredLinks.map(url => ({
        url,
        score: calculateRelevanceScore(url, row.model, row.brand)
    }));

    // Keep Top 50 highest relevance score links (Phần 4)

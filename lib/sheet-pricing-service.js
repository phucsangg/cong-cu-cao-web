...
async function searchProductLinks({ brand, model, limit = 20, fetchImpl = fetch, sheetName, sourceMap }) {
    ...
    try {
        ...
        const doubleCheckedResult = pricingCache.getSearchResult(model);
...
function extractPriceAdvanced($, html, model, url, referencePrice) {
    ...
    const isJunkNode = (el) => {
        const isRelated = $(el).closest('.related, .upsell, .cross-sell, .recommend, .suggested, .product-slider, .similar, .may-like, .swiper-slide, .swiper-container, .slick-slide, .slick-track, .owl-item, .owl-stage, .carousel, .slider, [class*="related"], [class*="upsell"], [class*="recommend"], [class*="similar"], [class*="slider"], [class*="carousel"], [class*="swiper"], [class*="slick"], [class*="owl"], aside, .sidebar, .aside, [class*="sidebar"], [class*="aside"], #column-left, #column-right, .widget, [class*="widget"], .latest, .hot, .featured, .bestseller, [class*="latest"], [class*="featured"], [class*="bestseller"], [class*="latest_product"], [class*="list_product"], [class*="product_module"], [class*="product-layout"]').length > 0;

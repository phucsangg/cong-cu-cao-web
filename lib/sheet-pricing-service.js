...
function collectPriceCandidates($, referencePrice) {
    ...
    selectorList.forEach((selector) => {
        $(selector).each((_, element) => {
            ...
            // Ignore accessories, parts, or wrong parses outside refined range
    ...
    // Only fallback to body-wide text matching if specific selectors found nothing
    if (candidates.length === 0) {
        $('body *').each((_, element) => {
            // Only parse leaf nodes to avoid text merging issues from parent containers
    ...
    if (refPrice && candidates.length > 0) {
        // Sort by proximity to reference price to exclude different models/accessories

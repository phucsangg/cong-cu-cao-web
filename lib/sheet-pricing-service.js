...
function isLikelyProductDetailUrl(url = '', model = '', brand = '') {
    ...
    // Do not crawl search engine pages, Google help docs, or excluded domains (like bepngocbao.vn, websosanh.vn)
    if (
        ...
        normalized.includes('websosanh.vn')
    ) {
        return false;
    }
    ...
    if (model) {
        if (hasConflictingModelSuffix(url, model, brand)) {
            return false;
        }
...
async function fetchHtml(url, { timeout = 15000, fetchImpl = fetch, userAgent = DEFAULT_USER_AGENT, retries = 2 } = {}) {
    return fetchQueue.add(() => fetchHtmlWithRetry(url, { timeout, fetchImpl, userAgent }, retries));
}
...
async function fetchHtmlWithRetry(url, options, retriesLeft) {
    ...
    try {
        ...
        if (retriesLeft > 0 && !isBlockingError) {
...
class CacheStore {
    ...
    save() {
        try {
            // Exclude temporary errors when writing to disk
            const copy = { ...this.data };
            delete copy.errors;
            fs.writeFileSync(this.filePath, JSON.stringify(copy), 'utf8');
        } catch (e) {}
    }
    ...
    getError(url) {
        const entry = this.data.errors?.[url];
        if (!entry) return null;
        const ageMs = Date.now() - entry.timestamp;
        if (ageMs > 10 * 60 * 1000) { // 10 minutes TTL
            delete this.data.errors[url];
            return null;
        }
        return entry.value;
    }

    getSearchResult(brand, model) {
        const key = `${String(brand || '').trim().toLowerCase()}:${String(model || '').trim().toLowerCase()}`;
        const entry = this.data.searchResults?.[key];
        if (!entry) return null;
        const ageMs = Date.now() - entry.timestamp;
        if (ageMs > 7 * 24 * 60 * 60 * 1000) { // 7 days TTL
            delete this.data.searchResults[key];
            this.save();
            return null;
        }
        totalCacheHits++;
    ...
    setSearchResult(brand, model, urls) {
        if (!this.data.searchResults) this.data.searchResults = {};
        const key = `${String(brand || '').trim().toLowerCase()}:${String(model || '').trim().toLowerCase()}`;
        this.data.searchResults[key] = {
            value: urls,
            timestamp: Date.now(),
        };
        this.save();
    ...
    getUrlsForModel(brand, model) {
        const key = `${String(brand || '').trim().toLowerCase()}:${String(model || '').trim().toLowerCase()}`;
        const entry = this.data.urlMap?.[key];
        if (!entry) return [];
        totalCacheHits++;
        return Object.values(entry).map(item => item.url);
    }

    setUrlForModelDomain(brand, model, domain, url) {
        if (!this.data.urlMap) this.data.urlMap = {};
        const key = `${String(brand || '').trim().toLowerCase()}:${String(model || '').trim().toLowerCase()}`;
        if (!this.data.urlMap[key]) this.data.urlMap[key] = {};
        this.data.urlMap[key][domain] = {
            url,
            timestamp: Date.now(),
        };
        this.save();
...
async function searchProductLinks({ brand, model, limit = 20, fetchImpl = fetch, sheetName, sourceMap }) {
    // 1. Kiểm tra cache ngay lập tức trước khi xếp hàng/khóa để tăng tốc độ tối đa
    const cachedResult = pricingCache.getSearchResult(brand, model);
    if (cachedResult) {
        if (sourceMap) {
    ...
    try {
        // Double-check cache sau khi nhận được khóa (tránh trường hợp luồng chạy trước vừa tìm xong và lưu cache)
        const doubleCheckedResult = pricingCache.getSearchResult(brand, model);
        if (doubleCheckedResult) {
            if (sourceMap) {
                doubleCheckedResult.forEach(url => sourceMap.set(url, 'Cache'));
            }
            return doubleCheckedResult;
        }
        ...
        for (const stepKws of steps) {
            for (const engine of engines) {
                if (Date.now() < (engineBlockedUntil[engine] || 0)) {
                    continue;
                }
                ...
                for (const q of stepKws) {
                    ...
                    if (engine === 'google') {
                        engineLinks = await getCachedSearchLinks('google', q, async (queryVal) => {
                            const urls = [];
                            const googlePages = [0, 40];
                            for (const start of googlePages) {
                                if (start > 0 && urls.filter(l => isLikelyProductDetailUrl(l, model, brand)).length >= 5) {
                                    break;
                                }
                                try {
                                    ...
                                    if (html.includes('google.com/recaptcha') || html.includes('detected unusual traffic')) {
                                        engineBlockedUntil.google = Date.now() + 10 * 60 * 1000;
                                        break;
                                    }
                        ...
                        engineLinks = await getCachedSearchLinks('bing', q, async (queryVal) => {
                            ...
                            for (const first of bingPages) {
                                if (first > 1 && urls.filter(l => isLikelyProductDetailUrl(l, model, brand)).length >= 5) {
                                    break;
                                }
                                try {
                                    ...
                                    if (html.includes('detected unusual traffic')) {
                                        engineBlockedUntil.bing = Date.now() + 10 * 60 * 1000;
                                        break;
                                    }
                                    const cheerio = getCheerio();
                                    const $ = cheerio.load(html);
                        ...
                        engineLinks = await getCachedSearchLinks('ddg', q, async (queryVal) => {
                            ...
                            try {
                                ...
                                if (err.message.includes('429') || err.message.includes('403')) {
                                    engineBlockedUntil.ddg = Date.now() + 10 * 60 * 1000;
                                }
                            }
                            return urls;
                        });
                        ...
                        engineLinks = await getCachedSearchLinks('coccoc', q, async (queryVal) => {
                            ...
                            try {
                                ...
                                if (err.message.includes('429') || err.message.includes('403')) {
                                    engineBlockedUntil.coccoc = Date.now() + 10 * 60 * 1000;
                                }
                            }
                            return urls;
                        });
        ...
        if (accumulatedLinks.length > 0) {
            pricingCache.setSearchResult(brand, model, accumulatedLinks);
        }
...
function extractPriceAdvanced($, html, model, url, referencePrice) {
    ...
    // Mark containers with junk headings (related products, same type/price, news, reviews, etc.)
    const junkHeadingKeywords = [
        'lien quan', 'cung loai', 'cung muc gia', 'cung danh muc', 'tuong tu',
        'co the ban thich', 'da xem', 'tieu bieu', 'ban chay', 'moi nhat',
        'tin tuc', 'bai viet', 'chuyen muc', 'tags', 'binh luan', 'nhan xet',
        'tin khuyen mai'
    ];

    $('h1, h2, h3, h4, h5, h6, .title, .heading, [class*="title"], [class*="heading"]').each((_, el) => {
        const text = normalizeVietnameseText($(el).text());
        const isJunkHeading = junkHeadingKeywords.some(keyword => text.includes(keyword));
        if (isJunkHeading) {
            // Also mark the heading element itself and its direct next sibling
            $(el).addClass('junk-container-marked');
            const nextSib = $(el).next();
            if (nextSib.length > 0 && !nextSib.find('h1').length) {
                nextSib.addClass('junk-container-marked');
            }

            let parent = $(el).parent();
            for (let i = 0; i < 3 && parent.length > 0; i++) {
                const name = parent[0].name;
                if (name === 'body' || name === 'html' || parent.attr('id') === 'product-detail' || parent.hasClass('product-detail')) {
                    break;
                }
                // DO NOT mark if it contains the main product title (h1)
                if (parent.find('h1').length > 0) {
                    break;
                }
                parent.addClass('junk-container-marked');

                // Also mark next sibling of parent if it doesn't contain h1
                const parentNext = parent.next();
                if (parentNext.length > 0 && !parentNext.find('h1').length) {
                    parentNext.addClass('junk-container-marked');
                }
    ...
    const isJunkNode = (el) => {
        const isJunk = $(el).closest([
            '.junk-container-marked',
            '.related', '.upsell', '.cross-sell', '.recommend', '.suggested', '.product-slider', '.similar', '.may-like',
            '.swiper-slide', '.swiper-container', '.slick-slide', '.slick-track', '.owl-item', '.owl-stage', '.carousel', '.slider',
            '[class*="related"]', '[class*="upsell"]', '[class*="recommend"]', '[class*="similar"]', '[class*="slider"]',
            '[class*="carousel"]', '[class*="swiper"]', '[class*="slick"]', '[class*="owl"]',
            'aside', '.sidebar', '.aside', '[class*="sidebar"]', '[class*="aside"]', '#column-left', '#column-right',
            '.widget', '[class*="widget"]', '.latest', '.hot', '.featured', '.bestseller',
            '[class*="latest"]', '[class*="featured"]', '[class*="bestseller"]', '[class*="latest_product"]', '[class*="list_product"]',
            '[class*="product_module"]', '[class*="product-layout"]',
            // Added junk contexts for reviews, blog, news, articles, library
            '[class*="review"]', '[class*="news"]', '[class*="blog"]', 'article:not(.product)', '[class*="article"]',
            '[class*="library"]', '[class*="thu-vien"]', '[class*="thuvien"]', '[class*="post"]', '[class*="bai-viet"]', '[class*="baiviet"]',
            // Added junk contexts for specs, description, title, heading, breadcrumbs
            'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'title', '.breadcrumb', '[class*="breadcrumb"]',
            '[class*="specs"]', '[class*="specification"]', '[class*="parameter"]', '[class*="technical"]', '[class*="thong-so"]', '[class*="thongso"]', '[class*="description"]', '[class*="mo-ta"]', '[class*="mota"]',
            '[class*="heading"]', '[class*="title"]',
            // Added junk contexts for promotional gifts/offers/combos
            '[class*="offer-product"]', '[class*="offer-type"]', '[class*="offer_detail"]', '[class*="offer-item"]', '[class*="offer_item"]',
            '[class*="gift-product"]', '[class*="gift_product"]', '[class*="gift-item"]', '[class*="gift_item"]',
            '[class*="promo-product"]', '[class*="promo_product"]', '[class*="promo-item"]', '[class*="promo_item"]',
            '[class*="bonus-product"]', '[class*="bonus_product"]', '[class*="bonus-item"]', '[class*="bonus_item"]'
        ].join(',')).length > 0;
        if (isJunk) return true;

        const isOriginalPrice = $(el).closest('del, [class*="old-price"], [class*="compare-at-price"], [class*="original-price"], [class*="price-old"], [class*="price-compare"], [class*="compare-price"]').length > 0;
    ...
    if (domain) {
        ...
        if (learnedSelector) {
            ...
            $mainContainer.find(learnedSelector).each((_, el) => {
    ...
    for (const sel of htmlSelectors) {
        let foundPrice = null;
        $mainContainer.find(sel).each((_, el) => {
            if (isJunkNode(el)) {
                return;
            }
            const val = $(el).attr('content') || $(el).attr('value') || $(el).text();
            const parsed = parseVietnamesePrice(val);
            if (isValid(parsed)) {
                console.log(`[DEBUG] HTML Selector matched: "${sel}" -> text: "${val}", parsed: ${parsed}`);
                foundPrice = parsed;
                return false;
            }
        });
    ...
    if (!isKocher) {
        ...
        const extractFromJson = (obj) => {
            ...
            if (obj['@type'] === 'Product') {
                const name = obj.name || '';
                if (name && !isModelMatch(name, model)) {
                    return;
                }
            }
    ...
    $mainContainer.find('[itemprop="price"]').each((_, el) => {
        if (isJunkNode(el)) return;
        const val = $(el).attr('content') || $(el).attr('value') || $(el).text();
        const parsed = parseVietnamesePrice(val);
        if (isValid(parsed)) {
...
async function extractProductPrice({ url, model, brand, referencePrice, fetchImpl = fetch, retries }) {
    ...
    if (!html) {
        ...
        try {
            html = await fetchHtml(url, { fetchImpl, timeout: 8000, retries });
            pricingCache.setHtml(url, html);
        } catch (error) {
            pricingCache.setError(url, `Fetch failed: ${error.message}`);
            return null;
        }
    }
    ...
    if (!verifyResult.valid) {
        pricingCache.setError(url, `Reject: ${verifyResult.reason}`);
        return null;
    }
...
async function processPricingRow({ row, deps = {} }) {
    const cleanModel = cleanModelSpecs(row?.model);
    ...
    const cachedUrls = pricingCache.getUrlsForModel(row.brand, cleanModel);
    if (cachedUrls && cachedUrls.length > 0) {
        ...
        // XML Sitemap discovery - fallback only if 0 discovered links
        const likelyLinks = discoveredLinks.filter(link => isLikelyProductDetailUrl(link, cleanModel, row.brand));
        let allSitemapLinks = [];
        if (likelyLinks.length === 0) {
            ...
            const sitemapLinksPromises = uniqueDomains.map(async (domain) => {
                try {
                    const urls = await discoverSitemapLinks(domain, cleanModel, row.brand, deps.fetchImpl || fetch);
                    urls.forEach(url => {
        ...
        allFilteredLinks = mergedLinks.filter((link) => isLikelyProductDetailUrl(link, cleanModel, row.brand));
    }
    ...
    const scoredLinks = allFilteredLinks.map(url => ({
        url,
        score: calculateRelevanceScore(url, cleanModel, row.brand)
    }));
                ...
                try {
                    ...
                    if (price && price >= 100000 && price <= 2000000000) {
                        ...
                        try {
                            ...
                            pricingCache.setUrlForModelDomain(row.brand, cleanModel, host, url);
                        } catch (e) {}
    ...
    const keywordsUsed = generateKeywords(row.brand, cleanModel, row.sheetName).slice(0, 3).join(', ');
    const pricingLogs = [];
    
    filteredLinks.forEach(url => {
        const engine = sourceMap.get(url) || 'Search Engine';
        const relScore = calculateRelevanceScore(url, cleanModel, row.brand);
        const matchEntry = matchedDetails.find(d => d.url === url);
        const priceFound = matchEntry ? matchEntry.price : null;
        let rejectReason = '';
        if (!matchEntry) {
            const recordedError = pricingCache.getError(url);
            rejectReason = recordedError || 'Model mismatch or price not found';
        }
    ...
    return {
        ...
        suggestedPrice: pricing.suggestedPrice,
        outlierRemoved: pricing.outlierRemoved,
        status: finalPrices.length > 0 ? 'success' : 'insufficient_prices',
        pricingLogs,
        stats
    };
}
...
async function readSheetRows({ appsScriptUrl, sheetUrl, sheetName, startRow, endRow, specificRows, specificRowsEnabled, scanToEndEnabled, fetchImpl = fetch }) {
    const isSpecificActive = specificRowsEnabled !== false && specificRows;
    const rowSet = isSpecificActive ? parseSpecificRows(specificRows) : null;
    const isScanToEnd = isSpecificActive && scanToEndEnabled === true;

    let finalStartRow = Math.max(3, parseInt(startRow, 10) || 3);
    let finalEndRow = endRow ? parseInt(endRow, 10) : null;

    if (rowSet) {
        const rowArr = Array.from(rowSet);
        finalStartRow = Math.max(3, Math.min(...rowArr));
        if (isScanToEnd) {
            finalEndRow = null;
        } else {
            finalEndRow = Math.max(finalStartRow, Math.max(...rowArr));
        }
    }
    ...
    if (rowSet && !isScanToEnd) {
        rawRows = rawRows.filter(row => rowSet.has(row.rowNumber));
    }
...
function startBackgroundPricingJob({
    ...
    let parsedStartRow = Math.max(3, parseInt(startRow, 10) || 3);
    let parsedEndRow = endRow ? parseInt(endRow, 10) : null;

    if (rowSet) {
        const rowArr = Array.from(rowSet);
        parsedStartRow = Math.max(3, Math.min(...rowArr));
        if (isScanToEnd) {
            parsedEndRow = null;
        } else {
            parsedEndRow = Math.max(parsedStartRow, Math.max(...rowArr));
        }
    }
    ...
    const job = {
        ...
        scanToEndEnabled,
        specificRows,
        rowsConcurrency: Math.max(1, parseInt(rowsConcurrency, 10) || Math.max(4, Math.min(16, (require('os').cpus().length || 4)))),
        linksConcurrency: Math.max(1, parseInt(linksConcurrency, 10) || 15),
        ...
        try {
            ...
            const fetchPromises = sheetNames.map(async (name) => {
                const data = await readSheetRowsFn({
                    ...
                    specificRowsEnabled: job.specificRowsEnabled,
                    scanToEndEnabled: job.scanToEndEnabled,
                    specificRows: job.specificRows,
                });
            ...
            const filteredMergedRows = (rowSet && !isScanToEnd)
                ? mergedRows.filter(row => rowSet.has(row.rowNumber))
                : mergedRows;

            const mergedRowsMapped = filteredMergedRows.map((row) => {
                        ...
                        try {
                            ...
                            const writePromises = Object.entries(updatesBySheet).map(async ([name, sheetUpdates]) => {
                                ...
                                try {
                                    ...
                                    sheetUpdates.forEach((u) => {
                                        const target = job.rows.find((r) => r.sheetName === u.sheetName && r.rowNumber === u.rowNumber);
                                        if (target) {
                                            target.writtenToSheet = true;
                                        }
                                        job.writeCount += 1;
                                        log(`Đã ghi thành công kết quả dòng ${u.rowNumber} [${u.sheetName}] về Google Sheet.`, 'success');
                                    });
                                } catch (rowErr) {
                                    // Put back failed updates to pending
                                    pendingUpdates.unshift(...sheetUpdates);
                                    log(`Ghi kết quả dòng ${sheetUpdates.map(u => u.rowNumber).join(', ')} [${name}] thất bại: ${rowErr.message}`, 'error');
                                    throw rowErr;
                                }
                            });
